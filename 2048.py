...
class Game2048(Frame):
    def __init__(self):
        ...
        self.commands={c.KEY_UP:lf.move_up,c.KEY_DOWN:lf.move_down,c.KEY_LEFT:lf.move_left,c.KEY_RIGHT:lf.move_right}
        self.grid_cells=[]
        self.init_grid()
        self.init_matrix()
    ...
    def key_down(self,event):
        ...
        if key in self.commands:
            self.matrix,changed=self.commands[reps(event.char)](self.matrix)
            if changed:
                lf.add_new_2(self.matrix)
                self.update_grid_cells()
                changed=False
                ...
                if lf.get_current_state(self.matrix)=="LOST":
                    self.grid_cells[1][1].configure(text="you",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
